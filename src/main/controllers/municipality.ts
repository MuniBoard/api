...
const inputMakesSense = (request: Request) => {
  const websiteURLIsValid = request.body.website
    ? validateWebsiteURL(request.body.website)
    : true;
...
const isValidCreationRequest = (request: Request) => {
  ...
  const containsValidCoordinates =
    ...
    request.body?.coordinates !== null
      ? typeof request.body?.coordinates === "object" &&
        containsValidLat &&
        containsValidLong &&
  ...
  return (
    containsBody &&
    containsValidName &&
    containsValidCoordinates &&
    containsValidWebsite &&
    objectContainsNoOtherKeys(request.body, ["name", "coordinates", "website"])
  );
};
...
const get = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const municipalities = getRepository(
...
const getSingle = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const municipality = getRepository(ObjectsManipulated.Municipality).get(
